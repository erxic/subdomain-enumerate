...
while True:
    ...
    for sub in subdomain_array:
        ...
        print(":", animated.__getitem__(index % animated.__len__()-1),
              f"({index}/{subdomain_array.__len__()})", end="\r")
        index += 1
    ...
    print((font.UNDERLINE + "info".upper()+font.ECD))
    print(
    ...
    user_args = str(input(font.CYAN +"press any key to continues or Press N to stop ?".upper())).upper()
    if user_args == "N":
        break
