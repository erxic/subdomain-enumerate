...
#import pyfiglet
#banner = pyfiglet.figlet_format("STALKER") 
...
def main():
    ...
    for sub in subdomain_array:
            try:
                nilai_ip = dns.resolver.resolve(f"{sub}.{domain}")
                if nilai_ip:
                    output_subdomain.append(f"{sub}.{domain}")
                    if f"{sub}.{domain}" in output_subdomain:
                        print((font.GREEN + font.BOLD + f"{sub}.{domain}  <==Valid!"))
                    else:
                        pass
...
##untuk kinerja lebih cepat 
#for t in subdomain_array:
...
print ((font.WARNING + f"waktu proses ={finish-start} detik"))

##pengulangan program
run = True
while(run==True):
    m =input("coba lagi? y/n:")
    if(m=="n"):
        run = False
    elif(m=="y"):
        domain(main()) 
    else:
        print("masukkan nilai yang tepat!!!")
